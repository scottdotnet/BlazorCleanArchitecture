...
namespace BlazorCleanArchitecture.Application.Authentication.Validators
{
    ...
    {
        public GenerateMFAQRCodeValidator(IMemoryCache cache, IApplicationDbContext context)
        {
            RuleFor(x => x.UserId)
                .MustAsync(async (id, cancellationToken) =>
                {
                    return await cache.GetOrCreateAsync(nameof(GenerateMFAQRCode), async e =>
                    {
                        return await context.Users.SingleOrDefaultAsync(x => x.Id == id, cancellationToken);
                    }) is not null;
                })
                .WithMessage("User does not exist.")
                .MustAsync(async (x, cancellationToken) => await Task.FromResult(cache.Get<Domain.User.User>(nameof(GenerateMFAQRCode)).MFAKey != Guid.Empty))
